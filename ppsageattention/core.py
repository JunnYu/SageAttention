...
try:
    ...
    print("[WARNING] Triton kernels are not available!")
    per_block_int8_triton = per_block_int8_varlen_triton = per_block_int8_hd96 = attn_h96_false = attn_h96_true = attn_false = attn_true = attn_false_varlen = attn_true_varlen = None

from qattn_fused import qk_int8_sv_f16_accum_f32_attn_per_warp
from qattn_fused import qk_int8_sv_f16_accum_f16_attn_per_warp, qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_per_warp
from qattn_fused import qk_int8_sv_f8_accum_f32_fuse_v_scale_attn_per_warp, qk_int8_sv_f8_accum_f32_fuse_v_scale_fuse_v_mean_attn_per_warp
from qattn_fused import qk_int8_sv_f16_accum_f16_attn_per_warp_buf, qk_int8_sv_f8_accum_f32_fuse_v_scale_attn_per_warp_buf
...
def sageattn(
    ...
    **kwargs: Any,
    ...
    - The tensors `q`, `k`, and `v` must have the dtype ``paddle.float16`` or ``paddle.bfloat16``
...
def sageattn_qk_int8_pv_fp16_triton(
    ...
    **kwargs: Any,
    ...
    ----
    - ``num_qo_heads`` must be divisible by ``num_kv_heads``. 
    - The tensors `q`, `k`, and `v` must have the dtype ``paddle.float16``, ``paddle.bfloat16`` or ``paddle.float32``.
    - All tensors must be on the same cuda device.
    ...
    assert dtype in [paddle.float16, paddle.bfloat16], "Input tensors must be in dtype of paddle.float16 or paddle.bfloat16"
    ...
    if smooth_k:
        km = k.mean(axis=seq_dim, keepdim=True)
        if return_lse:
            if tensor_layout == "NHD":
                lse_correction = paddle.matmul(q.transpose([0, 2, 1, 3]), km.transpose([0, 2, 3, 1])).squeeze(-1).cast(paddle.float32)
            else:
                lse_correction = paddle.matmul(q, km.transpose([0, 1, 3, 2])).squeeze(-1).cast(paddle.float32)
    else:
        km = None
...
def sageattn_varlen(
    ...
    **kwargs: Any,
    ...
    - ``num_qo_heads`` must be divisible by ``num_kv_heads``.
    - The tensors `q`, `k`, and `v` must have the dtype ``paddle.float16``, ``paddle.bfloat16`` or ``paddle.float32``.
    - The tensors `cu_seqlens_q` and `cu_seqlens_k` must have the dtype ``paddle.int32`` or ``paddle.int64``.
    - All tensors must be on the same cuda device.
    - `smooth_k` will introduce slight overhead but will improve the accuracy under most circumstances.
    """
    ...
    if smooth_k:
        km = k.mean(axis=0, keepdim=True) # ! km is calculated on the all the batches. Calculate over each individual sequence requires dedicated kernel.
        k -= km
